...
public class SchedulerJob : FullAggregateRoot<Guid, Guid>
{
    ...
    {
        LastScheduleTime = scheduleTime == DateTimeOffset.MinValue ? DateTimeOffset.UtcNow : scheduleTime;
    }
    ...
    public void UpdateLastRunDetail(TaskRunStatus taskRunStatus)
    {
        ...
        switch (taskRunStatus)
        {
            case TaskRunStatus.Running:
                LastRunStartTime = DateTimeOffset.UtcNow;
                break;
            case TaskRunStatus.Success:
            case TaskRunStatus.TimeoutSuccess:
                LastRunEndTime = DateTimeOffset.UtcNow;
                break;
    ...
    public void SetHttpConfig(SchedulerJobHttpConfigDto? dto)
    {
        ...
        HttpConfig ??= new();
        HttpConfig.SetConfig(dto.HttpMethod, dto.RequestUrl, dto.HttpParameters, dto.HttpHeaders, dto.HttpBody, dto.HttpVerifyType, dto.VerifyContent, dto.IsAsync);
    }
