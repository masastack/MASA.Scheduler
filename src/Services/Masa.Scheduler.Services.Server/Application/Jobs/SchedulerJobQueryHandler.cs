...
public class SchedulerJobQueryHandler
{
    ...
    public async Task JobListHandleAsync(SchedulerJobQuery query)
    {
        ...
        var key = CacheKeys.SCHEDULER_JOB + md5Key;

        var response = await _memoryCacheClient.GetAsync<SchedulerJobListResponse>(key);
        ...
        if(response == null)
        {
            ...
            response = new(total, totalPages, jobDtos, originList);

            await _memoryCacheClient.SetAsync(key, response, new CombinedCacheEntryOptions<SchedulerJobListResponse>()
            {
